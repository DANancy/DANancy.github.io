...
import { Archive,BookOpen,BrainCircuit,BriefcaseBusiness,CalendarDays,Coffee,Compass,Download,ExternalLink,GraduationCap,Lightbulb,Presentation,Store,Link as LinkIcon,Mail,MapPin,Network,Palette,PartyPopper,Send,Star,Terminal,Wrench,X } from "lucide-react";
...
import booksCharacter from "@/assets/web/character-books.png";
...
const characterPoses:Record<DesktopSection,StaticImageData>={overview:characterImage,about:aboutCharacter,work:workCharacter,community:communityCharacter,books:booksCharacter,fun:funCharacter,contact:contactCharacter};
...
export function DesktopPortfolio(){
  ...
  return <main className="desktop-shell" onPointerDown={(event)=>{if(active!=="overview"&&event.target===event.currentTarget)setActive("overview")}}>
    ...
    <section className={`desktop-window ${active==="overview"?"overview-window":"section-window"}`} aria-label={active==="overview"?"Portfolio overview":undefined} role={active==="overview"?undefined:"dialog"} aria-modal={active==="overview"?undefined:true}>
      ...
      {active==="overview"&&<div className="window-status"><span><i/> Ready</span><span><i className="amber-dot"/> Creative mode</span><span className="status-right">&#10019; Optimized</span></div>}
    </section>
    <Character key={active} active={active}/>
    {active!=="overview"&&<button className="desktop-floating-close" onClick={()=>setActive("overview")} aria-label={`Close ${active} window`}><X size={20}/></button>}
    <footer className="desktop-footer"><a href="https://www.linkedin.com" target="_blank" rel="noreferrer"><LinkIcon size={15}/> LinkedIn</a><span>&copy; 2026 Yangyang Cai</span></footer>
  </main>;
}
...
function Character({active}:{active:DesktopSection}){const[greeting,setGreeting]=useState(false);return <button className={`desktop-character character-${active}`} onClick={()=>setGreeting(value=>!value)} aria-label="Say hello to Yangyang">{greeting&&<span className="character-greeting" role="status">Hi!</span>}<Image src={characterPoses[active]} alt={`Yangyang character for ${active}`} width={393} height={373} priority/></button>}
...
function About({onOpen}:{onOpen:(section:DesktopSection)=>void}){return <div className="window-page about-page"><p className="page-pill">Professional profile</p><h2>Building the bridge between<br/><em>Data</em> and <strong>Intelligence.</strong></h2><p className="page-lead">Based in Melbourne, I am an <b>AI-Powered Data Engineer</b> dedicated to crafting robust data and AI solutions that solve real-world problems. I see data as more than numbers&mdash;it is the narrative of our digital lives, waiting for the right architecture to speak clearly.</p><h3>Working style</h3><div className="style-chips"><span><Wrench size={15}/> Practical</span><span><Terminal size={15}/> Technical</span><span><Lightbulb size={15}/> Curious</span></div><hr/><h3>Beyond the code</h3><div className="beyond-grid"><span><Palette/>Creative</span><span><Coffee/>Melbourne coffee</span><span><BookOpen/>Wide reader</span></div><div className="page-actions"><a className="solid-button" href="mailto:hello@yangyangcai.me"><Download size={17}/> Request my CV</a><button className="outline-button" onClick={()=>onOpen("contact")}><Mail size={17}/> Get in touch</button></div></div>}

function Work(){const ProjectIcons=[BriefcaseBusiness,Presentation,GraduationCap,Store];return <div className="window-page work-page"><h2>Selected Works</h2><p className="page-lead">A digital collection of professional work, learning initiatives, and practical projects.</p><div className="project-grid">{projects.map((project,index)=>{const Icon=ProjectIcons[index];return <article key={project.title}><div className={`project-icon ${project.tone}`}><Icon/></div>{project.href?<a className="card-link" href={project.href} target="_blank" rel="noreferrer" aria-label={`Visit ${project.title}`}><ExternalLink size={17}/></a>:<ExternalLink className="card-link muted-link" size={17}/>}<h3>{project.title}</h3><p>{project.description}</p><div>{project.tags.map(tag=><span key={tag}>{tag}</span>)}</div></article>})}</div><div className="work-status"><span>4 projects selected</span><span><i/> Synced</span><span>Last updated: recently</span></div></div>}
...
function Books(){return <div className="window-page books-page"><div className="book-focus"><Image src={bookImage} alt="A current book from Yangyang's reading shelf" width={320} height={430}/><div><p className="page-pill">Current focus</p><h2>Ideas worth<br/>staying with.</h2><div className="rating" aria-label="Four out of five stars"><Star/><Star/><Star/><Star/><Star className="muted-star"/></div><blockquote>&ldquo;Reading is how I borrow another person&apos;s mental model&mdash;and test it against my own.&rdquo;</blockquote></div></div><section className="digital-shelf"><p className="page-pill">Collections</p><h3>The Digital Shelf</h3><div><article><BrainCircuit/><b>Systems & data</b><span>Architecture, modelling, reliability</span></article><article><Lightbulb/><b>AI & society</b><span>Adoption, agents, collaboration</span></article><article><BookOpen/><b>Fantasy worlds</b><span>Imagination, cultivation, adventure</span></article></div></section></div>}

function Fun(){return <div className="window-page fun-page"><div className="fun-heading"><p className="page-pill">Personal atelier</p><h2>Just for Fun</h2><p>When I&apos;m not architecting data pipelines, I&apos;m usually at the potter&apos;s wheel or exploring digital realms.</p></div><div className="fun-grid"><section className="pottery-window"><h3>pottery.studio</h3><div className="pottery-images"><Image src={potteryOneImage} alt="Handmade pottery in progress" width={400} height={280}/><Image src={potteryTwoImage} alt="Colourful glazed pottery" width={400} height={280}/><Image src={potteryThreeImage} alt="Freshly shaped ceramic spoons" width={800} height={360}/></div><div className="mini-tags"><span>Wheel throwing</span><span>Glazing</span></div></section><section className="games-window"><h3>fun.bin</h3><GameCard image={itTakesTwoImage} title="It Takes Two" note="Co-op masterpiece"/><GameCard image={hogwartsLegacyImage} title="Hogwarts Legacy" note="Magical explorer"/><blockquote>&ldquo;Games are another form of problem solving&mdash;just with better graphics than a terminal.&rdquo;</blockquote><button>Play next quest</button></section></div></div>}
